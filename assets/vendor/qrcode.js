...
//
// Vendored for TypechoPaid to render offline payment QR codes without
// relying on any third-party CDN or network request at runtime.
//
//---------------------------------------------------------------------
...
var qrcode = function () {
  ...
  var QRUtil = function () {
    ...
    _this.getLostPoint = function (qrcodeInst) {
      var moduleCount = qrcodeInst.getModuleCount();
      var lostPoint = 0;

      for (var row = 0; row < moduleCount; row += 1) {
        for (var col = 0; col < moduleCount; col += 1) {
          var sameCount = 0;
          var dark = qrcodeInst.isDark(row, col);
          for (var r = -1; r <= 1; r += 1) {
            if (row + r < 0 || moduleCount <= row + r) continue;
            for (var c = -1; c <= 1; c += 1) {
              if (col + c < 0 || moduleCount <= col + c) continue;
              if (r == 0 && c == 0) continue;
              if (dark == qrcodeInst.isDark(row + r, col + c)) sameCount += 1;
            }
          }
      ...
      for (var row2 = 0; row2 < moduleCount - 1; row2 += 1) {
        for (var col2 = 0; col2 < moduleCount - 1; col2 += 1) {
          var count = 0;
          if (qrcodeInst.isDark(row2, col2)) count += 1;
          if (qrcodeInst.isDark(row2 + 1, col2)) count += 1;
          if (qrcodeInst.isDark(row2, col2 + 1)) count += 1;
          if (qrcodeInst.isDark(row2 + 1, col2 + 1)) count += 1;
      ...
      for (var row3 = 0; row3 < moduleCount; row3 += 1) {
        for (var col3 = 0; col3 < moduleCount - 6; col3 += 1) {
          if (qrcodeInst.isDark(row3, col3)
            && !qrcodeInst.isDark(row3, col3 + 1)
            && qrcodeInst.isDark(row3, col3 + 2)
            && qrcodeInst.isDark(row3, col3 + 3)
            && qrcodeInst.isDark(row3, col3 + 4)
            && !qrcodeInst.isDark(row3, col3 + 5)
            && qrcodeInst.isDark(row3, col3 + 6)) {
            lostPoint += 40;
          }
        }
      }
      ...
      for (var col4 = 0; col4 < moduleCount; col4 += 1) {
        for (var row4 = 0; row4 < moduleCount - 6; row4 += 1) {
          if (qrcodeInst.isDark(row4, col4)
            && !qrcodeInst.isDark(row4 + 1, col4)
            && qrcodeInst.isDark(row4 + 2, col4)
            && qrcodeInst.isDark(row4 + 3, col4)
            && qrcodeInst.isDark(row4 + 4, col4)
            && !qrcodeInst.isDark(row4 + 5, col4)
            && qrcodeInst.isDark(row4 + 6, col4)) {
            lostPoint += 40;
          }
        }
      }
      ...
      for (var col5 = 0; col5 < moduleCount; col5 += 1) {
        for (var row5 = 0; row5 < moduleCount; row5 += 1) {
          if (qrcodeInst.isDark(row5, col5)) darkCount += 1;
        }
      }
  ...
  var QRMath = function () {
    ...
    for (var i3 = 0; i3 < 255; i3 += 1) { LOG_TABLE[EXP_TABLE[i3]] = i3; }
  ...
  function qrPolynomial(num, shift) {
    ...
    _this.multiply = function (e) {
      ...
      for (var i2 = 0; i2 < _this.getLength(); i2 += 1) {
        for (var j = 0; j < e.getLength(); j += 1) {
          out[i2 + j] = (out[i2 + j] || 0) ^ QRMath.gexp(QRMath.glog(_this.getAt(i2)) + QRMath.glog(e.getAt(j)));
        }
      }
    ...
    _this.mod = function (e) {
      ...
      for (var i3 = 0; i3 < e.getLength(); i3 += 1) { num2[i3] ^= QRMath.gexp(QRMath.glog(e.getAt(i3)) + ratio); }
    ...
    var createBytes = function (buffer, rsBlocks) {
      ...
      for (var i4 = 0; i4 < maxDcCount; i4 += 1) {
        for (var r2 = 0; r2 < rsBlocks.length; r2 += 1) {
          if (i4 < dcdata[r2].length) { data2[index] = dcdata[r2][i4]; index += 1; }
        }
      }
      for (var i5 = 0; i5 < maxEcCount; i5 += 1) {
        for (var r3 = 0; r3 < rsBlocks.length; r3 += 1) {
          if (i5 < ecdata[r3].length) { data2[index] = ecdata[r3][i5]; index += 1; }
        }
      }
    ...
    _this.renderTo2dContext = function (context, cellSize) {
      ...
      for (var row = 0; row < length; row++) {
        for (var col = 0; col < length; col++) {
          context.fillStyle = _this.isDark(row, col) ? '#000000' : '#ffffff';
...
(function (factory) {
  ...
  else { this.qrcode = factory(); }
}(function () { return qrcode; }));
